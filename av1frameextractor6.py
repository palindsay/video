...
def is_blurry(image_path, threshold, downscale_factor, edge_percent):
    """
    ...
    try:
        with Image.open(image_path) as img:
            ...
            # Normalize the edge variance by the image size
            normalized_variance = edge_variance / (width * height)
...
def process_video(video_path, output_dir, num_frames, blur_threshold, downscale_factor, edge_percent):
        ...
        for i in range(num_frames * 2):
            ...
            try:
                ...
                if os.path.exists(frame_path):
                    if not is_blurry(frame_path, blur_threshold, downscale_factor, edge_percent):
                        extracted_frames += 1
                    else:
...
def main(input_dir, output_dir, num_frames, max_videos, blur_threshold, downscale_factor, edge_percent):
    ...
    with ThreadPoolExecutor() as executor:
        list(tqdm(
            executor.map(lambda v: process_video(v, output_path, num_frames, blur_threshold, downscale_factor, edge_percent), video_files),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--blur_threshold", type=float, default=1e-6, help="Threshold for blur detection (default: 1e-6)")
    parser.add_argument("--downscale_factor", type=int, default=1, help="Factor to downscale images before blur detection (default: 1)")
    parser.add_argument("--edge_percent", type=float, default=0.2, help="Percentage of strongest edges to consider (default: 0.2)")
    ...
    print(f"Edge percent: {args.edge_percent}")
    if args.max_videos:
        print(f"Processing up to {args.max_videos} videos")
    
    main(args.input_dir, args.output_dir, args.num_frames, args.max_videos, args.blur_threshold, args.downscale_factor, args.edge_percent)
