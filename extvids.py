...
def extract_frame(input_file, output_file, timestamp):
    cmd = [
        ...
        '-ss', str(timestamp),
        '-i', input_file,
        '-vf', 'scale=iw*2:ih*2',  # Upscale by 2x for higher quality
        '-frames:v', '1',
...
def process_video(input_file, output_dir, num_frames, mode):
    ...
    if mode == 'evenly':
        ...
        evenly_frames = num_frames // 2
        random_frames = num_frames - evenly_frames
        
        interval = (duration - start_time) / evenly_frames
        even_timestamps = [start_time + i * interval for i in range(evenly_frames)]
        random_timestamps = [random.uniform(start_time, duration) for _ in range(random_frames)]
        
        timestamps = even_timestamps + random_timestamps
        random.shuffle(timestamps)  # Shuffle to mix evenly and randomly selected frames
...
if __name__ == "__main__":
    ...
    parser.add_argument("mode", choices=['evenly', 'random', 'mixed'], help="Frame selection mode")
