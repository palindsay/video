...
def extract_frames(video_path, output_dir, num_frames, start_frame=250, end_frame=250):
    video_info = get_video_info(video_path)
    total_frames = video_info['num_frames']
    fps = video_info['fps']
    duration = video_info['duration']
    
    if total_frames <= start_frame + end_frame:
        print(f"Warning: Video {video_path} is too short after excluding frames. Skipping.")
        return

    start_time = start_frame / fps
    end_time = duration - (end_frame / fps)

    # Calculate frame indices for even distribution
    even_timestamps = np.linspace(start_time, end_time, num_frames)
    
    # Calculate frame indices for random distribution
    random_timestamps = sorted(np.random.uniform(start_time, end_time, num_frames))
